...
```
cd harness && python3 -m pytest -q          # expect: 94 passed
```
...
| Risk allocation | `risk_allocator.py` | Formula implemented and unit-tested (`value_density = expected_risk / cost`). **`cost` is never populated from real data anywhere in the codebase** — every caller either omits it (defaults to 1.0, i.e. cost-blind) or would need to wire `effort.EffortLedger.average_tokens` in themselves. If you see code elsewhere assuming cost-aware ranking is "live," that's a hallucination to correct, not a fact to build on. |
...
| Credential redaction | `base_agent._redact_headers` | Yes, wired into `_user_prompt()`, unit-tested. **Does not cover response/request bodies** — a token in a JSON body (not a header) still reaches the prompt. Known, disclosed gap, not fixed. |
