...
A Burp Suite extension that hands a selected request/response to a local
Ollama-backed multi-agent harness: one coordinator model decides which
narrow specialist agents (SQLi, XSS, IDOR, SSRF, auth) are worth running
on that exchange, each specialist reports structured findings, and the
extension shows the aggregated result in a Burp tab.

**Scope note:** this is a triage aid, not a scanner replacement. It reads
one exchange at a time and proposes what to check next in Repeater; it
does not send traffic to the target on its own, and it does not generate
working exploit payloads. For use only against applications you're
authorized to test — same assumption Burp itself runs on.
