...
class SubdomainTakeoverValidator(Validator):
    ...
    def plan(self, finding: Finding, exchange: HttpExchange) -> Any:
        ...
        plan_id = f"takeover_test_{hashlib.sha256(plan_data.encode()).hexdigest()[:16]}"
        ...
        return TestPlan(
            ...
            description=f"Subdomain takeover validation for {finding.vulnerability_class}",
            parameters={
                "finding_class": finding.vulnerability_class,
                "target_url": exchange.url,
                "confidence": finding.confidence,
            },
            expected_outcome="Confirm an unclaimed third-party resource fingerprint",
