    ...
    try:
        ...
        for f in findings:
            ...
            rows.append((host, exchange.url, exchange.method, agent_name, f.vulnerability_class,
                         f.severity, f.confidence, f.summary, f.basis, f.review_verdict,
                         int(f.confirmed), fingerprint, model, prompt_version, now))
        conn.executemany(
            """INSERT OR IGNORE INTO findings
               (host, url, method, agent, vulnerability_class, severity,
                confidence, summary, basis, review_verdict, confirmed, fingerprint,
...
def all_host_findings(url: str) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            """SELECT url, vulnerability_class, severity, confidence, summary
               FROM findings WHERE host = ? ORDER BY created_at ASC""",
            (host,),
        ).fetchall()
    finally:
        conn.close()
...
def save_identity(identity) -> None:
    ...
    try:
        conn.execute(
            ...
            (identity.id, identity.name, identity.role.value if hasattr(identity.role, "value") else identity.role,
