...
from role_crawl import RoleSession, _template_ids
...
def _fake_discovery(paths):
    class _FakeDisc:
        ...
        async def discover(self):
            from api_surface_discovery import SurfaceResult, Route
...
class ActiveDiscoveryInjectionTests(unittest.TestCase):
    ...
    def test_discovery_augments_surface(self):
        ...
        with patch("crawler.crawl", _fake_crawl_empty()), \
             patch("api_surface_discovery.SurfaceDiscovery", _fake_discovery(["/api/tickets/1", "/api/admin/users"])), \
    ...
    def test_discovery_off_by_default_keeps_js_only(self):
        with patch("crawler.crawl", _fake_crawl_empty()), \
             patch("api_surface_discovery.SurfaceDiscovery", _fake_discovery(["/api/should_not_appear"])), \
...
class BuildEngagementTests(unittest.TestCase):
    ...
    def test_builds_prioritised_worklist_over_discovered_surface(self):
        ...
        with patch("crawler.crawl", _fake_crawl_empty()), \
             patch("api_surface_discovery.SurfaceDiscovery", _fake_discovery(["/api/admin/users", "/api/tickets/1"])), \
             patch("httpx.AsyncClient.request", _fake_probe(matrix)):
