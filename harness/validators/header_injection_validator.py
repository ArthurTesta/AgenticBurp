...
class HeaderInjectionValidator(Validator):
    ...
    def plan(self, finding: Finding, exchange: HttpExchange) -> Any:
        ...
        plan_id = f"header_inj_test_{hashlib.sha256(plan_data.encode()).hexdigest()[:16]}"
        ...
        return TestPlan(
            ...
            description=f"Header injection validation for {finding.vulnerability_class}",
            parameters={
                "finding_class": finding.vulnerability_class,
                "target_url": exchange.url,
                "confidence": finding.confidence,
            },
            expected_outcome="Confirm CRLF injection into a response header via a query parameter",
